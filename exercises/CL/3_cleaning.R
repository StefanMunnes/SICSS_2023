...
# If your laptop has trouble with these operations, reduce the number of articles with
...
# cnn_data <- cnn_data %>%
#   mutate(first_name=str_extract(authors, "\\w+")) %>% 
#   mutate(last_name=str_extract(authors, "\\s(\\w*)"))
...
cnn_data <- cnn_data %>% 
  mutate(updated = ifelse(str_detect(timestamp, "Updated"), TRUE, FALSE)) %>% 
  mutate(weekday = str_extract(timestamp, "(?<=EDT, |EST, ).{3}")) %>%
  mutate(date = as.Date(str_extract(timestamp, "\\b\\w+ \\d{1,2}, \\d{4}\\b"), format = "%b %d, %Y"))
