...
textdata2 <- readRDS("FOX_complete.Rds")

install.packages("plyr")
library("plyr")

textdata3 <- rbind.fill(textdata, textdata2)

# create corpus, tokens and dfm (just first ten documents)
corpus <- corpus(textdata3[1:100, ], text_field = "body")
head(summary(corpus))
...
convert(fcm_pp, to = "data.frame") |>
  dplyr::select(doc_id, "matter") |>
  dplyr::filter(matter > 0) |>
  dplyr::arrange(desc(matter))
...
tmod_lda <- textmodel_lda(dfm_pp, k = 12)
